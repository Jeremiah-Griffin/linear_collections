...
pub unsafe fn clear_to<T, const CAPACITY: usize>(
    list: &mut RawStackList<T, CAPACITY>,
    limit: NonZero<usize>,
) {
    let mut i = 0;
...
///SAFETY: Undefined Behavior if accessed beyond CAPACITY *OR* into uninitialized element.
pub unsafe fn get_mut<T, const CAPACITY: usize>(
    list: &mut RawStackList<T, CAPACITY>,
    index: usize,
) -> &mut T {
    //SAFETY: upheld by caller
...
pub unsafe fn insert_at<T, const CAPACITY: usize>(
    list: &mut RawStackList<T, CAPACITY>,
    index: usize,
    item: T,
) {
...
pub unsafe fn iter_to<'a, T, const CAPACITY: usize>(
    list: &'a RawStackList<T, CAPACITY>,
    limit: usize,
...
///It must be guaranteed that all items <= index are initialized.
///Creates an iterator over all elements 0 to `limit`, inclusive.
pub unsafe fn iter_mut_to<'a, T, const CAPACITY: usize>(
    list: &'a mut RawStackList<T, CAPACITY>,
    limit: usize,
...
pub unsafe fn remove<T, const CAPACITY: usize>(
    list: &mut RawStackList<T, CAPACITY>,
    index: usize,
    length: usize,
) -> T {

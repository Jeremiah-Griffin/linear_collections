...
///SAFETY: UB if accessed beyond CAPACITY *OR* into uninitialized element.
pub unsafe fn get_mut<'a, T, const CAPACITY: usize>(list: & 'a mut RawStackList<T, CAPACITY>, index: usize) -> & 'a T{
    //SAFETY: upheld by caller
    unsafe { list.array.get_unchecked_mut(index).assume_init_ref()}

...
pub fn with_heap_capacity_does_not_change_length() {
    assert_eq!(FatVec::<bool, 0>::with_heap_capacity(0).len(), 0);
    assert_eq!(FatVec::<bool, 50>::with_heap_capacity(50).len(), 0);
}
...
pub fn clear_drops() {
    ...
    svec.push(Dropper::new(third_was_dropped.clone()));
    svec.push(Dropper::new(fourth_was_dropped.clone()));
    svec.push(Dropper::new(fifth_was_dropped.clone()));
...
pub fn remove_empty() {
    ...
    let mut has_stack = FatVec::<&str, 10>::with_heap_capacity(100);
